...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-[#003366]">Support Messages</h1>
                <p class="text-[#4A5568] mt-1">Ask questions and get help from our team</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div id="messages-container" class="p-6 space-y-4 max-h-[600px] overflow-y-auto">
            @forelse($thread->messages as $message)
                <div class="flex {{ $message->isFromUser() ? 'justify-end' : 'justify-start' }}">
                    <div class="flex items-start max-w-[70%] {{ $message->isFromUser() ? 'flex-row-reverse' : '' }}">
                        <!-- Avatar -->
                        <div class="flex-shrink-0 {{ $message->isFromUser() ? 'ml-3' : 'mr-3' }}">
                            @if($message->sender->avatar)
                                <img src="{{ $message->sender->avatar }}" alt="{{ $message->sender->name }}" class="w-10 h-10 rounded-full">
                            @else
                                <div class="w-10 h-10 rounded-full {{ $message->isFromUser() ? 'bg-[#0066CC]' : 'bg-gray-400' }} flex items-center justify-center">
                        ...
                        <!-- Message Bubble -->
                        <div>
                            <div class="flex items-center mb-1 {{ $message->isFromUser() ? 'justify-end' : '' }}">
                            ...
                            <div class="rounded-lg px-4 py-3 {{ $message->isFromUser() ? 'bg-[#0066CC] text-white' : 'bg-gray-100 text-[#003366]' }}">
                                <p class="text-sm whitespace-pre-wrap">{{ $message->message }}</p>
                                @if($message->attachment_path)
                                    <a href="{{ route('messages.download', $message) }}" class="mt-2 inline-flex items-center text-sm {{ $message->isFromUser() ? 'text-white hover:text-gray-200' : 'text-[#0066CC] hover:text-[#003366]' }}">
                            ...
                            <p class="text-xs text-[#4A5568] mt-1 {{ $message->isFromUser() ? 'text-right' : '' }}">
        ...
        @else
            <div class="p-6 border-t border-gray-200 bg-gray-50 text-center">
                <p class="text-[#4A5568]">This conversation has been closed. Contact an administrator if you need to reopen it.</p>
            </div>
        @endif
    </div>
</div>
